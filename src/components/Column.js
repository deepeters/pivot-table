...
class Column extends React.Component {
    ...
    _getTableColumn() {
        ...
        for (const key in clientData) {
           const stateGroup = clientData[key];
           const tempObj = { grandTotal: null };
    
           for (const keyName in data) {
            const categoryObjs = data[keyName];
              ...
              categoryObjs.map((obj, index) => {
                ...
                // Populate column object sums from clientData (using variables from Data)
      ...
      render() {
        const stateTitle = Object.keys(clientData).map(key => (
          <div key={key}>
